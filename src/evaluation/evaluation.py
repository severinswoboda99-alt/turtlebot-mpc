...
def correct_arucos(x, y, yaw, max_jump=0.3, max_yaw_jump=np.deg2rad(45)):
    x_c = [x[0]] 
    y_c = [y[0]] 
    yaw_c = [yaw[0]] 
    for i in range(1, len(x)): 
        ...
        dy = y[i] - y_c[-1] 
        dist = np.hypot(dx, dy) 
        dyaw = np.arctan2(np.sin(yaw[i] - yaw_c[-1]), np.cos(yaw[i] - yaw_c[-1])) 
        if dist > max_jump or abs(dyaw) > max_yaw_jump:
            x_c.append(x_c[-1] + np.clip(dx, -max_jump, max_jump)) 
            y_c.append(y_c[-1] + np.clip(dy, -max_jump, max_jump)) 
            yaw_c.append(yaw_c[-1] + np.clip(dyaw, -max_yaw_jump, max_yaw_jump)) 
        else: 
            x_c.append(x[i]) 
            y_c.append(y[i]) 
            yaw_c.append(yaw[i]) 
        
    return np.array(x_c), np.array(y_c), np.array(yaw_c) 
    
def ema_smooth(x, alpha=0.15): 
    out = [x[0]] 
    for i in range(1, len(x)): 
...
if __name__ == "__main__":
    bag_path = "xxx"            # rosbag2 folder, onboard
    bag_path_camera = "xxx"     # rosbag2 folder, camera/aruco detection
    bag_path_sim = "xxx"        # rosbag2 folder, gazebo simulation
    ...
    sim = read_bag(bag_path_sim, "/traveled_path", "nav_msgs/msg/Path")
    ...
    sx, sy, syaw = extract_traveled(sim)
    ...
    xa_c, ya_c, yawa_c = correct_arucos(xa_n, ya_n, yawa_n) 
    xa_s = ema_smooth(xa_c, alpha=0.12) 
    ya_s = ema_smooth(ya_c, alpha=0.12) 
    xa_f = savgol_filter(xa_s, window_length=11, polyorder=2) 
    ya_f = savgol_filter(ya_s, window_length=11, polyorder=2)
    ...
    plt.legend()
    plt.axis('equal')
    plt.title(f"TurtleBot Path Comparison, Path {P}, Run {run}")
    plt.grid(linewidth = 0.5)
    ...
    plt.figure()
    plt.plot(px, py, linestyle='--', label='Planned Path', color='b')
    plt.plot(-ya_f, -xa_f, label='ArUco Trajectory', color='orange')
    plt.xlabel("X [m]")
    plt.ylabel("Y [m]")
    plt.legend()
    plt.axis('equal')
    plt.title(f"ArUco Path Comparison, Path {P}, Run {run}")
    plt.grid(linewidth = 0.5)
    plt.savefig(f"aruco_comp_R{run}_P{P}.pdf")
    plt.show()
    
    # Aruco trajectory comparison full
    plt.figure()
    plt.plot(px, py, linestyle='--', label='Planned Path', color='b')
    plt.plot(tx, ty, label='Robot Trajectory', color='crimson')
    plt.plot(-ya_f, -xa_f, label='ArUco Trajectory', color='orange')
    ...
    plt.legend()
    plt.axis('equal')
    plt.title(f"Combined Path Comparison, Path {P}, Run {run}")
    plt.grid(linewidth = 0.5)
    plt.savefig(f"combined_comp_R{run}_P{P}.pdf")
    plt.show()
    ...
    plt.plot(tx, ty, label='Robot Trajectory', color='crimson')
    plt.plot(-ya_f, -xa_f, label='ArUco Trajectory', color='orange')
    plt.plot(sx, sy, label='Simulation Trajectory', color='limegreen')
    plt.xlabel("X [m]")
    plt.ylabel("Y [m]")
    plt.legend()
    plt.axis('equal')
    plt.title(f"Full Path Comparison, Path {P}, Run {run}")
    plt.grid(linewidth = 0.5)
    plt.savefig(f"full_comp_R{run}_P{P}.pdf")
    plt.show()